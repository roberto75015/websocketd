...
type Config struct {
	...
	Raw            bool     // Use raw communication (send data char by char)
	...
	AnyOrigin      bool     // If set, explicitly allows upgrades from any origin (the current default); silences the no-policy startup warning.
	Headers        []string
	HeadersWs      []string
	HeadersHTTP    []string
