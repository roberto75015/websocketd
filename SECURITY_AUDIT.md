...
- **A2 ADDRESSED (2026-09-01, commits `fc02550`/`fe6181d`)** — without changing today's behavior: a prominent stderr banner now appears whenever no origin policy is configured, explaining the exposure, the options (`--sameorigin`, `--origin=…`, and the new `--anyorigin` opt-out that explicitly retains the permissive default), and announcing that a future version will default to `--sameorigin`. The default flip itself remains deferred to that future release.
...
- **A4 FIXED (teardown half)** — commit `14374dc`: children run in their own process group; teardown signals the group and finishes with a SIGKILL sweep; survivors must `setsid`. Regression: `TestPROC013`. The *session-linger* half (a live client keeps a session open while a grandchild holds the pipes) is **WONTFIX (maintainer decision, 2026-09-01)**: the pipe cannot distinguish "daemonized leftovers" from "a worker still producing output", and closing the session on direct-child exit would break the legitimate fork-a-worker-then-exit pattern. A client who stays connected keeps the slot by choice; teardown on disconnect is now complete.
...
- **A6, A7, A8, A9 — FIXED (follow-up round, same day)** (A7's scheme-warning gate and A8's TOCTOU resolved as WONTFIX-with-rationale, see below): A6 → commit `84e2b86` rejects negative `--maxframesize` (issue #472); A7 → commit `e6b978c` makes `--origin` port matching strict with an explicit `:*` wildcard (issue #473 — behavior change, migration in CHANGES); A8 → commit `d61c4e1` adds `--socketmode` to pin Unix socket permissions, applied immediately after bind (issue #474; the stale-socket TOCTOU noted alongside is **WONTFIX** — every exploitation path requires a local attacker with write access to the socket's directory, at which point they can already block startup, delete or replace files there, and interpose on anything else in that directory; the atomic alternative, bind-to-temp-path + `rename(2)`, is recorded here for whoever ever needs it); A9 → commit `090f42d` documents the Host-derived `SERVER_NAME`/`SERVER_PORT` trust boundary and the default `PATH` passthrough (issue #475 — kept as intended virtual-host semantics, matching `net/http/cgi`).

## A7 and A10 status (2026-09-01)

- **A7 — CLOSED.** The port-wildcard half was the real risk and is fixed by strict port matching (`e6b978c`, issue #473). The remaining schemeless-entry scheme wildcard (matches both http and https) is intended and documented, and the warning for it staying `--ssl`-gated is correct: a plain-HTTP server effectively only ever sees http origins (browsers block mixed-content `ws://` from https pages), so scheme ambiguity only bites under TLS — exactly when the warning fires.
- **A10 — all items closed out.** Fixed: superfluous-WriteHeader noise → `e33b67b`; `GetURLInfo` panic guard → `711b356`; 10MB binary buffer → `b1919cd` (64KB, pipe-granular); `UNIQUE_ID` crypto-random → `9190052`. WONTFIX by decision: unbounded single-line stdout buffering (the child is operator-chosen and can exhaust memory by itself — the pipe framing grants it no power it lacks, and capping would only truncate data); `--reverselookup` synchronous DNS (opt-in and rare; recommendation if it ever matters: timebox `net.LookupAddr` via `net.DefaultResolver.LookupAddr(ctx, …)` with a ~2s context so a hostile reverse zone can't park connection goroutines); static directory listings (operator-selected root, standard static-server behavior; an opt-out flag can be added if requested).

## Recommended fix order (historical — superseded by the remediation sections above)
